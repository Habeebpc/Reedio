...
from podcast_app.response import ErrorResponse
    ...
    def __new__(self, exc, context):
        ...
        if response is not None:
            if response.status_code == HTTP_401_UNAUTHORIZED:
                ...
                return ErrorResponse(status_code=HTTP_401_UNAUTHORIZED,
                                     message=str(exc))
            else:
                try:
            ...
            return ErrorResponse(status_code=response.status_code,
                                 message=message)
        return ErrorResponse(message="Server is facing technical" +
